...
- **Deformers**: per-part `deform: {taper, twist, bend}` for gestural shapes (tree trunks, bent pipes, hull wedges). Needs along-axis resolution: `options.segments: [sx,sy,sz]` on box, `options.heightSegments` on cylinder/cone. See `specs/fixtures/deform-test.json`.
- **Lofts are in-spec now**: `type: "loft"` with `loft: {stations, startPoint?, endPoint?, mirror?}` — the hullgen math as a standard part that composes with materials/breakup/terrain (see `specs/fixtures/loft-test.json`, the whale). Use `cli/hullgen.mjs` only when you need gltf/--lua export output.
- **Box modelling is in-spec with `type: "polyMesh"`**: tagged polygon faces support select/extrude/inset/translate/scale/rotate plus a welded half-cage mirror modifier. Use it for directional character anatomy and other forms that cannot be expressed as radial primitives or profile lofts. The current contract deliberately excludes region extrusion and subdivision; see `specs/fixtures/polymesh-test.json` and `docs/primitives.md`.
- **Poses**: spec-level `poses: {name: {joint: [rx,ry,rz]}}` + `pose` on the spec or a module/prefab placement (joints resolve with the placement prefix). Reference articulated figure: `specs/fixtures/pose-test.json`.
- **Joint clips**: top-level `clips: {name: {duration?, instances?, tracks}}` compiles degree-based joint rotations to quaternion keyframes (and supports position tracks), appears in the viewer clip controls, and exports with GLTF/GLB. See `specs/examples/segmented-figures.json`.
...
- The older `animations` behavior/state block remains separate from validated joint `clips`; use `clips` for portable articulated animation.
...
- Terrain / landscape: `scene.terrain` block — `method` heightfield / marching-cubes / cloth-drape, `display` primitives|terrain|both. Normals flip by default now; only set `flipNormals: false` to opt out. See the four `specs/examples/landscape-*.json` examples.
...
7. Inspect with `node cli/inspect-model.js <spec.json> [out-dir] [port]` (out-dir defaults to `/tmp/samdin-inspect`; pass a `[port]` to run parallel inspections). For joint- and face-scale work add `--target <part>` — close-up orbit sets around matching parts. Sweeps catch stance and proportion; part-scale defects need part-scale framing (the segmented-figures capsule-head defects were invisible in sweeps).
